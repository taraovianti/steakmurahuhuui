...
?>
	...
	<head>
		<title>Masuk || Steak Murah Uhuuui</title>
		<meta charset="UTF-8">
	...
	<body>
		...
		if (isset($_POST['register'])) {
			...
			$password = password_hash($_POST['password'], PASSWORD_BCRYPT); // Sebaiknya menggunakan hashing untuk password
			$status = 'aktif'; // Sesuaikan dengan status yang diinginkan
			$level = $_POST['level']; // Sesuaikan dengan level user yang diinginkan

			$query = "INSERT INTO tb_user (username, password, status, id_level) VALUES ('$username', '$password', '$status', '$level')";
